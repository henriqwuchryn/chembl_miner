...
if not os.path.exists(f"{datasets_path}/gd.csv"):
    data = DatasetWrapper().load_raw_dataset(
        f"{datasets_folder}/{filename}", general_columns, target_column
    )
...
if outlier_detection == 1:
    ...
    score_df_cv_clean = pd.DataFrame(
        {"r2": r2_cv_clean, "rmse": rmse_cv_clean, "mae": mae_cv_clean},
        index=["score_cv_clean"],
    )
    ...
    score_df_train_clean = pd.DataFrame(
        {"r2": r2_train_clean, "rmse": rmse_train_clean, "mae": mae_train_clean},
        index=["score_train_clean"],
    )
