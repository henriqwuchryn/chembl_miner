...
data.describe()
threshold_quantile = np.sum(data.y_train <= 6.0) / len(data.y_train)
...
scoring = {  # defining scoring metrics for optimization
    ...
    "quantile": metrics.make_scorer(
        lambda y_true, y_pred: metrics.mean_pinball_loss(
            y_true, y_pred, alpha=threshold_quantile
        ),
...
if test_model == 1:
    ...
    quantile_test = metrics.mean_pinball_loss(
        data.y_test, y_pred_test, alpha=threshold_quantile
    )
